    ...
    const initFirebase = async () => {
      try {
        ...
        if (typeof __initial_auth_token !== 'undefined') {
          await signInWithCustomToken(appAuth, __initial_auth_token);
        } else {
...
const AuctioneerRoom = ({ room, userId, db, showPopup, setRoom }) => {
  ...
  useEffect(() => {
    ...
    const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
    const q = query(collection(db, 'artifacts', appId, 'public', 'data', 'rooms'), where('auctioneerId', '==', userId), where('active', '==', true));
  ...
  const addPlayer = async () => {
    ...
    try {
      const roomDocRef = doc(db, 'artifacts', __app_id, 'public', 'data', 'rooms', room.id);
  ...
  const handleCopyLink = () => {
    const roomLink = `${window.location.origin}?room=${room.id}`; // Simple deep-link-like approach
